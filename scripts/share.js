document.addEventListener('DOMContentLoaded', function() {
    ...
    uploadForm.addEventListener('submit', async function(e) {
        ...
        try {
            const serverResponse = await fetch('https://api.gofile.io/getServer', { headers: { 'Authorization': `Bearer ${goFileToken}` }});
            const serverData = await serverResponse.json();
            if (serverData.status !== 'ok') throw new Error('无法获取上传服务器');
            ...
            if (result.status !== 'ok') throw new Error(result.message || '文件上传失败');
